...
监听、版本、密钥数量、转换开关、上游凭据字段名以 [`../scripts/precheck.ps1`](../scripts/precheck.ps1) 为准（WSL：[`../scripts/precheck.sh`](../scripts/precheck.sh)）。协议路径以该版本官方文档为准。CPA Usage Keeper 是用量看板，不配进 Agent。
...
跑 precheck。失败则停。端到端最小请求仍按目标客户端。
