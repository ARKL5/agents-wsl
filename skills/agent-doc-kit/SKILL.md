...
目标根路径、点名的面、模式。写盘以 00 的「点名的面」表为准。
...
完成：对照表已交用户；audit 或只读面在此停。

### 5. 设计落点

仅 `bootstrap` / `reshape` 且该面允许写盘。按本仓事实画文件表。缺结构时打开 [`shapes/`](shapes/) 当骨架，按本仓事实填进已有文件；禁止整文件覆盖。

完成：用户同意文件表；或用户要求停。

### 6. 确认后写入

展示摘要 → 用户确认 → 写入。改 skill 或协议文档时遵守 `writing-for-agents`。

完成：磁盘与确认稿一致。

### 7. 自检

对照 00「完成」。另查：空 claims、旧词别名、未写入口菜单。
