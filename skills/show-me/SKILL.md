...
- For a visual UI, layout, state comparison, or a concept too dense for Mermaid, write one focused HTML file — a diagram, an infographic, or a short slide deck, whichever fits the point. Use real labels and data; support desktop and mobile. Write it under `/tmp/show-me-<slug>.html`, then open it:

```bash
wslview "$(readlink -f /tmp/show-me-<slug>.html)"
```

Report the absolute path. If `wslview` is missing, stop and report — no substitute open command.
